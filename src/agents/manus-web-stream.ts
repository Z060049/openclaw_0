...
export function createManusWebStreamFn(cookieOrJson: string): StreamFn {
  let options: ManusWebClientOptions;
  try {
  ...
  const client = new ManusWebClientBrowser(options);
  ...
  return (model, context, streamOptions) => {
    ...
    const run = async () => {
      try {
        ...
        if (!responseStream) {
          throw new Error("Manus API returned empty response body");
        }
        ...
        console.log(`[ManusWebStream] Stream completed. Content length: ${accumulatedContent.length}`);
